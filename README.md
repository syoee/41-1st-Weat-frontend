...
### 1. 장바구니 페이지
- 제품의 img 클릭, 수량 선택 후 바로 구매, 장바구니 버튼 클릭 시 백엔드 서버로 유저 데이터 전송
    - 상세 페이지에서 옵션, 수량 선택 후 장바구니 버튼 클릭 시 백엔드 서버로 유저 데이터 전송
- 유저 정보 확인 후 백엔드 서버에 저장된 장바구니 데이터 불러와서 출력
- 수량 증감 버튼으로 장바구니 상품 수량 설정 및 수량에 따른 금액 변동 기능
- 삭제 버튼으로 장바구니 상품 삭제 기능
- 상품들의 총 합계 기능

### 2. 결제 완료 페이지
- 유저 데이터 수신으로 해당 유저의 주문 내역 출력

---------------------------------------

Reference

해당 프로젝트는 정육각 사이트를 참조하여 학습 목적으로 제작하였습니다. <br>
이 코드를 활용하여 이득을 취하거나 무단 배포 할 경우 법적으로 문제 될 수 있습니다.
